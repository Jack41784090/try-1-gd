...
- **Demo scenes** in `scenes/demos/` — run with F6:
  ...
  - `warrior_rig_2_cutscene_demo.tscn` — tests `warrior_rig_2` (new-proportion rig) inside a real cutscene, driving **`GroupPlayback` directly (no `EventChain`)**. The cutscene is a single `CinematicGroup` `.tres` (`scenes/demos/warrior_rig_2_cutscene.tres`) attached to the demo's `cutscene` `@export` — that resource is the only source/authoring surface (edit in the inspector, no JSON, no runtime export key). Wiring: pre-spawns `warrior_rig_2` instances into `stage_view.rigs` so the pipeline uses them instead of `WarriorRigFactory`/`warrior_rig.tscn`; reuses `VnPresenter._on_instruction_fired` for stage dispatch (sets `character_ids_in_chain` directly, takes over `timeline_complete` since the presenter's default handler assumes a `current_chain`). `GroupPlayback` (`src/strategy/ui/vn/group_playback.gd`) is driven unmodified so engine edits are exercised directly. Rig art hot-reloads (polls bone .svg mtimes every 0.4s). F6 to play: SPACE/click advances gates / fast-forwards, R replays. Headless self-test (`DisplayServer.get_name() == "headless"`) auto-advances every gate and asserts completion. Usage: `godot --headless --path . scenes/demos/warrior_rig_2_cutscene_demo.tscn`. Script: `src/demos/warrior_rig_2_cutscene_demo.gd`
