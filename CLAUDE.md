...
- **Demo scenes** in `scenes/demos/` — run with F6:
  ...
  - `scenes/animation_test.tscn` — F6 harness for `warrior_rig_2.tscn` (new-proportion rig). **`warrior_rig_2.tscn` ships its textures baked in** (see "Baked editor textures" below), so the rig shows fully textured **in the editor** and animation poses can be previewed live by scrubbing `AnimPlayer` — no need to run the harness just to see a pose. The harness still instances the rig, applies a textured `WarriorRigConfig` (default `wcr_adventurer_rachelle.tres`, swappable in inspector — `apply_config` now *updates* the baked sprites), cycles behaviors via keys `1-8`/`←→`, `R` replays, **`E` cycles facial expressions** (from the `expressions` export — neutral/angry by default). Edit skeleton + animations in `warrior_rig_2.tscn`, then F6 `animation_test.tscn`. **Live texture hot-reload**: polls modified-times (0.4s) of the config `.tres`, each bone texture's source file, **and each face/expression overlay texture** (eye_l/eye_r/mouth/brows); edits to the SVG art or to the config/expression slots re-apply immediately without restart. Script: `src/demos/animation_test.gd`
...
- **Animation** (`src/animation/`): 5-layer system — Clips→iExpression→AnimAction→Behavior→WarriorAnimController. `WarriorRig` (warrior_rig.gd) runs in **dual mode**: (a) **baked** rigs (`warrior_rig_2.tscn`) ship a `Sprite2D` per bone (and `Face` under the `Head` bone) so the rig is textured in the editor — `apply_config()` *updates* those baked sprites in place (recompute `display_scale`/`offset`/`z_index`) and `_build_placeholder_body()` is skipped; (b) **legacy** rigs (`warrior_rig.tscn`) have no baked sprites — `apply_config()` spawns `top_level` Polygon2D placeholders/Sprite2Ds synced to bones each frame in `_process()`. `WarriorRigConfig/Factory` for per-class configs. **Inspector-driven texturing** (`WarriorRig` is `@tool`): besides the runtime `apply_config()`, the rig exports `config` (drop a full `WarriorRigConfig` to replace the placeholder/baked art) plus **auto-derived `character_name` + `emotion` dropdowns** — `_get_property_list()` builds live `PROPERTY_HINT_ENUM`s scanned from the asset folders, so picking a folder name (`rachelle`, `faust1`, …) + emotion (`neutral`/`wide`/`blink`, …) textures the rig from `assets/rig_textures/<name>/` with that emotion's face overlays. When `config` is *also* set, its bone sizes/offsets are kept and only the textures are overlaid (so a warrior_rig_2 proportion config can be retextured by name). **Live editor preview**: on baked rigs (warrior_rig_2) picking a `character_name`/`emotion` in the inspector retextures the baked sprites immediately (`_ready` + the property setters call `_apply_inspector_config()` under `Engine.is_editor_hint()` when `_has_baked_sprites()`); emotion preview requires `character_name` to be set (the folder is the source of the emotion variants). Legacy/placeholder rigs are not previewed in-editor (their `top_level` sprites need the `_process` sync, which stays editor-disabled along with animation). The scan + folder→config build live in **`RigTextureLibrary`** (`rig_texture_library.gd`): `character_names()`/`emotion_names()` (emotions = `face/eyes_<emotion>.svg` tokens, neutral-first), `build_config(name, emotion, base=null)`, `apply_textures()` (maps bone SVG stems → config slots, prefers `face/head_base_*` over `head.svg`, falls back to the neutral face variant). Setting `character_name`/`emotion` at runtime live-reapplies.
- **Baked editor textures**: `warrior_rig_2.tscn` has each bone's texture baked as a `Sprite2D` child of the `Bone2D` (named `<Bone>Sprite`) with `z_as_relative = false` and an explicit `z_index` (bone-child sprites paint in skeleton-tree order, which is *not* the desired draw order, so `WarriorRig._compute_baked_z_order()` assigns absolute z by sorting on `(config size.z, BONE_DRAW_ORDER rank)` — reproducing the legacy layering; runtime `_replace_limb()` reuses the same z). Because the sprites are real bone children, Godot's editor animation preview moves them — **scrub `AnimPlayer` to see textured poses live**. (Re)generate with `godot --headless --path . --script res://tools/bake_rig_scene.gd` — a `SceneTree` tool that instantiates the scene (no `_ready`, so no placeholders), reuses `WarriorRig.limb_target_size()`/`limb_display_scale()`/`_compute_baked_z_order()` to author the sprites from `resources/character/wcr_adventurer_rachelle.tres` (Rachelle = the default look), reparents `Face` under `Head`, **creates the `EyeL`/`EyeR`/`Mouth`/`Brows`/`HairBack` overlay sprites** (replacing the pre-split single `Eyes` node), repaths/renames the inert legacy `Face/Eyes:*` tracks to `Face/EyeL`, **detaches `FaceBlend` from the AnimTree output** so animations can't override expression textures, **wires `root.config`/`character_name="rachelle"`/`emotion="neutral"`** into the scene (so the @tool inspector preview keeps the proportions and the emotion dropdown works), and re-saves the `.tscn`. Re-run after changing Rachelle's textures or `*_size` values.
- **Facial expressions (texture-swap)**: facial features are full-canvas overlay sprites layered on the head **base** texture (head SVG with eyes/mouth/brows removed). The eyes are **split into separate `Face/EyeL` + `Face/EyeR` sprites** (near/far) so each can be swapped independently (winks, asymmetry); plus `Face/Mouth`, `Face/Brows`, `Face/HairBack`. In the baked `warrior_rig_2.tscn` the `Face` node is parented **under the `Head` bone** (`Skeleton2D/.../Head/Face`) so it tracks the head in editor and at runtime with no per-frame sync; `apply_config()` sets neutral overlays from `WarriorRigConfig` face slots (`eye_l_texture`/`eye_r_texture`/`mouth_texture`/`brows_texture` + `default_expression`, plus the `hair_back_texture` body slot — the back hair, rendered behind the body) and skips `_fit_face_to_head()` (that legacy helper, which top_level-synced `Face` to the Head bone with the head's display scale, now only runs for non-baked rigs whose `Face` sits at the rig root). `WarriorRig` resolves Face/feature refs by name (`find_child`) so both layouts work. `iExpression` (`expression.gd`) is a set of per-feature `Texture2D`s (`eye_l`/`eye_r`/`mouth`/`brows`; null = leave that feature unchanged); `WarriorRig.set_expression(expr)` swaps `Sprite2D.texture` per feature, and `set_expression_by_name(id)` resolves a named expression against the applied config's `expressions` array / `default_expression`. **The face is texture-swap-driven, NOT animated**: `bake_rig_scene.gd` disconnects the AnimTree `FaceBlend` (EyeAnim+MouthAnim) from the blend-tree output (`OutputAdd` input 1) so the leftover `EyeAnim`/`MouthAnim` tracks can't overwrite expression textures each frame; the inert legacy `Face/Eyes:*` frame-tracks are repathed to `Face/EyeL`. **Gotcha**: a config's `default_expression` is re-applied at the end of `_apply_config_internal`, so a folder/emotion build (`RigTextureLibrary`) and the `animation_test` reload both **null `default_expression`** to stop it snapping the face back to neutral over the chosen overlays. Expression `.tres` in `resources/animation/expressions/<character>/`. **Source art** is one layered head SVG (features as labelled groups in an Inkscape "Face" layer); `tools/export_face_features.py` (see SVG section) slices it into `face/{head_base,eye_l,eye_r,mouth,brows}_<emotion>.svg`. Preview/iterate via `animation_test.tscn` (`E` to swap) or the rig's `character_name`/`emotion` inspector dropdowns. **Texture sizing** is config-driven: `WarriorRigConfig` has per-bone `*_size` Vector3 + `*_offset` Vector2 fields (defaults match `WarriorRig.BONE_DISPLAY_SIZES`/`BONE_OFFSETS`, so untouched configs render identically). The `*_size` vector packs render size in `x`/`y` (px) and **draw order in `z`** (the sprite's `z_index`, higher draws in front, overriding `BONE_DRAW_ORDER` tree order). `_replace_limb()` scales each sprite to `target_size / texture_size` and applies `size.z` as `z_index`; a zero size x/y falls back to the rig constant. Set per-config sizes to fit a character's proportions (e.g. `warrior_rig_2`)
- **Rig Art Style**: 2D anime SD (super deformed) flat vector — clean 2px black outlines, solid color fills, no gradients or cross-hatching, 1:2.5 head-to-body ratio, **3/4 right-facing profile** (big eye on LEFT/near, small eye on RIGHT/far, nose bump on right edge). Z-order: Right* limbs drawn behind (far), Left* limbs in front (near). SVG textures in `assets/rig_textures/<class>/` (15 bones × 7 classes = 105 SVGs). Canvas demo copies in `scenes/demos/canvas/svgs/rig/<class>/`. Generator: `python3 tools/generate_sd_svgs.py`. Grain shader overlay: `assets/shaders/canvas/rugged_grain.gdshader` (grain + scratches + vignette + sepia)
  - **SVG clip baking** (`tools/bake_svg_clips.py`): Detailed hand-authored heads (e.g. `rachelle/_head.svg`) live as `_*.svg` sources; the bake tool emits Godot-loadable `*.svg`. Godot's ThorVG renderer (1) ignores `<use>` inside `<clipPath>` and (2) silently drops clip shapes buried in nested `<g>` wrappers — the latter let dark "shadow" gradients overflow as black smudges around the eyes. The tool resolves clip `<use>` refs, then **flattens** each `<clipPath>` to bare shapes with all ancestor `<g>`/`<use>` transforms composed down. Usage: `python3 tools/bake_svg_clips.py` (all `_*.svg`) or pass specific paths. Re-run after editing any `_*.svg`. **Auto face-split**: when a baked source contains an Inkscape `Face` layer (a layered head like `_head.svg`), the bake also runs `export_face_features.export()` on it (lazy import; `has_face_layer()`/`export_faces()`), so one `bake_svg_clips.py` (or its `--watch`) regenerates both the baked head and the `face/` feature/emotion overlays
  - **Face feature export** (`tools/export_face_features.py`): splits one layered head SVG into per-feature, **per-emotion** overlay SVGs for the texture-swap expression system. Authoring model: inside the Inkscape **"Face"** layer the features are **labelled groups** (`eye_l`/`eye_r` → eyes, `lips` → mouth, `brow_l`/`brow_r` → brows; `FEATURE_GROUPS` map), and **each feature group holds one sub-group per emotion** labelled by emotion name (e.g. `eye_l` → `{neutral, wide}`) — so all emotions live in one file. The tool discovers emotion names from those sub-groups and emits, per emotion, `face/{eye_l,eye_r,mouth,brows}_<emotion>.svg` — the **left and right eyes are separate overlays** (`eye_l`/`eye_r` are their own `FEATURE_GROUPS` entries) so each can be swapped independently (each feature group collapsed to that emotion's sub-group, falling back to its `neutral` sub-group when the emotion is absent — `select_emotion_subgroup`, which also **force-clears Inkscape's `display:none`** from the kept sub-group so a group you toggled hidden in the layers panel still renders in its overlay), plus a single emotion-independent `face/head_base_neutral.svg` (head minus the feature groups + base details `shadow`/`nose`/`blush`) and `face/hair_back_neutral.svg` (the `HairBack` **layer** via `FEATURE_LAYERS`). Every output keeps the **full source viewBox** so overlays align with no manual placement. **Eyes and brows are clipped to the base face**: `CLIP_TO_BASE_FEATURES` (`eye_l`/`eye_r`/`brows`) retains the container's face-silhouette `clip-path` for them (resolving its `<use>` of the `Base` group into `<defs>` *before* the base is stripped, so the clip stays self-contained); other features have that ancestor clip stripped. Reuses `bake_svg_clips.py`'s `<use>`-resolution + clip-flatten. Usage: `python3 tools/export_face_features.py [--watch] [src...]` (default `assets/rig_textures/rachelle/_head.svg`), or just run/`--watch` `bake_svg_clips.py` which now calls it automatically. To add an emotion (e.g. `angry`), author a same-named sub-group inside the feature groups you want to change and re-bake
...
- **Stage Scenery / Set Dressing** (`src/strategy/ui/stage/`): world-space backdrops + props (trees, pillars, banners, podium, the great map) rendered in a `Scenery` Node2D inside the SubViewport (drawn behind `WarriorContainer`; per-prop `z_index` overrides). `StageProp` (stage_prop.gd) + `StageSet` (stage_set.gd) are `Resource`s: a `StageSet` bundles a backdrop (`backdrop_svg_path`/position/scale/z/parallax) and `Array[StageProp]` (each `prop_id`, `svg_path`, `position`, `scale`, `z_index`, `flip_h`, `modulate`, **`parallax`** <1 = drifts slower/farther, `svg_scale`). `StageView.apply_stage_set()` clears the layer and builds a `Sprite2D` per prop via `SvgLoader.load_svg()` (`src/utils/svg_loader.gd`, shared runtime SVG→ImageTexture loader); also `add_prop/remove_prop/move_prop/modulate_prop/set_prop_visible/set_backdrop/reload_scenery_textures`. Parallax is a cheap per-frame offset in `_process` keyed off `parallax < 1.0` (default 1.0 props untouched). `StagePresenter` exposes the same methods. The `Scenery` node is defined in scenes that use it and lazily created elsewhere, so other StageView scenes need no edits. `background_image` is now null-safe (`get_node_or_null`). Assets: flat SD-style SVGs in `assets/scenery/` via `python3 tools/generate_scenery_svgs.py` (tree, pillar, banner, podium, bench, great_map, backdrop_chamber, backdrop_forest). Example set: `resources/stage_sets/parliament_chamber.tres`
- **Visual Novel** (`src/strategy/ui/vn/`): `EventChain` triggers via `event_chain_path` in results. `VnPresenter` is stage-aware (speech bubble on rig or fallback textbox). `DialogueInstruction` (instructions/dialogue_instruction.gd) extends `CinematicInstruction` with speaker_name, line_spoken, after_id. `GroupPlayback` processes `CinematicGroup` trees (parallel/sequential/auto-gate). `CharacterInstruction` (`Action` enum: MOVE/FACE/BEHAVIOR/SPAWN/SHOW/HIDE/**EXPRESSION** + StageAnchor) — `EXPRESSION` swaps the rig's face via `StagePresenter.set_character_expression()` → `WarriorRig.set_expression_by_name()` (expression id resolved against the rig config's `expressions`); **playing an animation and changing an expression are separate, combinable actions** — author a `BEHAVIOR` and an `EXPRESSION` instruction in the same parallel `CinematicGroup` to do both at once (build helpers `beh()`/`expr()` in `tools/build_parliament_cutscene.py`). `DialogueInstruction.expression_override` also routes through the expression path (no longer mis-mapped to a behavior). `CameraInstruction` (screen position panning), `SceneryInstruction` (instructions/scenery_instruction.gd — ADD/REMOVE/MOVE/MODULATE/SHOW/HIDE/SET_BACKDROP set dressing during playback; dispatched by `VnPresenter._execute_scenery`, needs no `GroupPlayback` change since it fires through the generic instruction path). `CinematicGroup`/`CinematicInstruction` `@export` all playback fields (`children`, `occupation`, `target_screen_position`, `anchor`/`anchor_offset`) so a group round-trips through `ResourceSaver.save()`/`load()` as a standalone reusable `.tres` cutscene (no `EventChain` wrapper needed) — feed the loaded `CinematicGroup` straight to `GroupPlayback.load_group()`
...
- `assets/scenery/` — flat SD-style backdrop/prop SVGs (tree, pillar, banner, podium, bench, great_map, backdrop_chamber, backdrop_forest); generator `tools/generate_scenery_svgs.py`. Stage sets in `resources/stage_sets/`
