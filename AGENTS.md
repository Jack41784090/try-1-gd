...
Guidance for AI coding agents working in this repo. Assumes no prior context.
...
- Viewport: 1920×1080
- Tactical combat core originated in a Roblox/TypeScript project (`life-is-roblox`); `README.md` covers that migration and is now mostly historical — trust this file over it.
...
Enabled (`project.godot` `[editor_plugins]`): `GDQuest_GDScript_formatter`, `_Godot-IDE_`, `curved_lines_2d`, `godot_ai`, `log`, `yard`.
- `log` addon exposes global `LogGd`; this project's own logger is `class_name MyLog` (`src/singletons/log.gd`, renamed from `Log` to avoid the clash). Prefer `LogGd` in new code — `MyLog` remains for existing call sites.
...
- **Curve domain convention** (get this right when authoring/migrating a curve): `Curve.sample()` uses whatever domain/tangents are stored — it does **not** recompute from a "linear" mode flag at load time.
  1. **Domain must cover the real input range**, ~2x headroom. Raw stats (`STAT` source) cluster 0.6–2.5 across `resources/combat/classes/base_stats/*.tres` → `max_domain = 5.0`. Resolved `Reality` values run wider (esp. MUL-composed): `Force`≈10, `Precision`/`Bravery`≈15, `Maneuver`≈16, `Mana`≈20, `Spirituality`≈22 (see `resources/combat/formula/*.tres`, any `PotencyObj.curve`). `PotencyObj`/`DamageTranslation` curves clamp silently past domain (no assert) — re-derive the domain per-`Reality`, don't reuse `5.0` by habit
  2. **To reproduce a flat multiplier `y = weight * x` exactly**: points `Vector2(0,0)` and `Vector2(max_domain, weight*max_domain)`, tangent mode `1` (linear) on both, with tangent **value** set to `weight` (not `0.0`) — the mode flag alone doesn't force a line; a `0.0` tangent with mode=linear Hermite-interpolates into an S-curve instead. On a multi-point piecewise-linear curve, each interior point's `tangent_in`/`tangent_out` = its *adjacent* segment's slope (differs across a kink) — verify empirically before trusting a hand-authored `.tres`
- `_limits = [min_value, max_value, min_domain, max_domain]` in the `.tres`; `max_value` **does** clamp `sample()`'s output — set it above the curve's actual peak, not left default
...
- `ClockSystem`, `SquadActingSystem`, `SquadTravelSystem`, `BattleResolutionSystem`, `ActivityRunSystem` — each a `Node` added under `main.tscn`'s `Systems` root by `main.gd._enter_system()`
...
- `SquadActingSystem` (merged from the former `SquadBeingSystem`+`SquadAISystem`) owns the squad registry AND AI decisions: a squad is AI-controlled iff it carries a `StrategySquadBrain` on its `StrategySquadResource.brain` (attached by whoever spawns it — e.g. `MonsterSpawnSystem`); `register_squad()` is the single registration step. Each hour it runs `brain.decide()` BEFORE emitting `squad_turn`, so listeners always read the fresh `current_activity_type` (no connect-ordering constraint)
- **Convoys are real squads**: `CaravanEconomySystem` trade matching builds a MERCHANT caravan via `CaravanBridge.create_caravan_squad` (guards as real warriors, cargo in `cargo.manifest`, `caravan-courier` brain attached) and emits `shipment_dispatched(move)`; `main.gd` registers it and delivery happens when the squad physically arrives (max one convoy per (thing, route) while in transit). `EconomyMove` is a dispatch record with a `squad` ref — no countdown; the legacy C# path keeps its own `CsEconomyMove.TurnsRemaining` clock
- **Map view**: `SquadMapView` (`src/strategy/ui/map/`) instanced as `World/EntityRoot/Map` in `main.tscn` renders the authored `prototype_map.tscn` markers plus one `SquadToken` per traveling squad (convoys, monsters — anything moving), interpolated along its route from `travel_progress_updated`. Pure view: `main.gd` wires the signals and injects `squad_resolver`/`distance_resolver` callables
- When a signal handler needs to reach a system the emitting system isn't allowed to know about (e.g. resolving `ActivityResult.combat_target_squad_id` to a `StrategySquad` needs `SquadActingSystem`, but neither `ActivityRunSystem` nor `BattleResolutionSystem` may reference it), that bridging logic lives in `main.gd`, not in either system
- Still prototype-stage: `main.gd._run_prototype_tests()` runs scripted assertions inline against `main.tscn` (call it from `_ready()` to run them). `scenes/demos/interactive_main.tscn` (`src/demos/interactive_main.gd`) is the terminal driver: it instantiates `main.tscn`, calls `load_debug_scenario()` and reads stdin commands — see `tools/start_main.sh`
- `main.gd` is `@tool` so it can be edited/opened safely in-editor (guards on `Engine.is_editor_hint()`); the `@export var DEBUG` toggle (off by default) controls whether `_ready()` auto-boots `debug_scenario`, a swappable `DebugScenario` Resource (`src/debug_scenario.gd`) defaulting to `resources/strategy/scenarios/debug/stress-test-trade.tres` (a 100-location trade-network stress test — swap in `prototype-sandbox.tres` for the small 2-location sandbox instead). `debug_scenario`'s Inspector field is hidden via `_validate_property()` whenever `DEBUG` is off. `DebugScenario` bundles a `GameScenario` + `Array[StrategySquad]` and its `prepare()` rebuilds the fields that don't round-trip through `.tres` (`GameScenario.triggerable_manager`, `StrategySquad.current_location_id`, `Population.people` — the last rebuilt from `population_config` same as `GameScenario._setup_economy()` does) — kept separate so `main.gd`'s production entry point, `load_scenario(scenario, squads)`, never depends on test-only data. `prototype-sandbox.tres` is generated from `MainTestScenario` (`src/main_test_scenario.gd`) by `tools/bake_debug_scenario.gd`; `stress-test-trade.tres` is generated from `StressTestScenario` (`src/stress_test_scenario.gd`) by `tools/bake_stress_test_scenario.gd` — re-run the relevant one after changing its source data
...
### Coding Rules
- When debugging using print statements, use LogGd to print instead of print().
...
- **Signals over direct calls for composed Resources**: a game-logic Resource held as a plain property on a Node (never a scene child) announces state changes via its own signal — custom, or inherited `changed`/`emit_changed()`. The owning Node connects/awaits; neither side calls into the other, and no intermediary Node shuttles between them. Examples: `ReactiveStat.changed` (`src/test_reactive_stat.gd`), `_squad.inventory.changed` (`manage_squad/inventory_panel.gd`, `unit_item.gd`, `inventory_tab.gd`), `SquadBattle.battle_completed` (already covered under Tactical Combat above)
- **MVP (View/Presenter) is a sanctioned exception, not the default**: reach for a separate Presenter only when a UI screen's orchestration is complex enough to warrant it — `strategy/ui/{market,travel,shop,scouting,stage,vn}` and `strategy/ui/presenter.gd` earn it. Everywhere else, default to composition + signal-announced state (`src/squad_battle/` dropped its Presenter tier for exactly this reason — see Tactical Combat above)
...
- **Don't use `preload`/untyped vars to work around "class not found"/"identifier not declared" errors** on a brand-new `class_name` script — that's a stale `.godot/global_script_class_cache.cfg`, not a real ordering constraint. Fix by forcing a real editor filesystem scan (plain `--headless --quit <scene>` only *reads* the cache, never writes it — looks fixed only if an earlier session already warmed it): `godot-mono --headless --editor --path . --quit-after 3` (~5-10s; "Scan thread aborted" on exit is harmless, cache is written before that). Normal static typing (`var x: NewClass`, `NewClass.new()`) then resolves immediately, even in the files that just failed. **Don't export RefCounted** types. **Don't use `class_name` for inner classes**
...
- **No single-use functions**: don't extract a function/method unless it's called from more than one site project-wide. A one-off block stays inline; if it needs a name for clarity, put a one-line comment above it instead of a signature. Same for `connect()`-target lambdas — inline `func(...)` at the call site with a `# _on_whatever: ...` comment, not a named method, unless genuinely reused. Exempt: engine-invoked entry points (`_ready`, `_process`, `_input`, `_draw`, ...), signal handlers wired via editor signal (not `connect()`), and `@tool`/exported inspector methods. Not exempt: ordinary single-call-site helpers in the same script — inline those
