...
func _can_execute_action(action: StrategicAction, situation: StrategicSituation) -> bool:
	# Validates whether an action can actually be performed in the current situation
	# Two checks: (1) does the location support this activity type? (2) can the action resolve its params?
	# e.g., TRAVEL → location-independent, but can_resolve checks if a destination exists
	# e.g., RECRUIT → location-dependent, must check location.has_activity_type(RECRUIT)
	var at = action.activity_type
	# 1. These activity types don't require the location to explicitly list them
