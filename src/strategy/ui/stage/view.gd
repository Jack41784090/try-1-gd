class_name StageView extends Control
	...
	for i in warriors.size():
		...
		if rig and is_instance_valid(rig):
			rig.position = Vector2(
				i * MARCH_SPACING_X - (warriors.size() * MARCH_SPACING_X * 0.5),
...
func _update_march(delta: float) -> void:
	for rig in rigs.values():
		...
		if rig.position.x > STAGE_WIDTH * 0.5:
			rig.position.x -= STAGE_WIDTH
...
func focus_camera_on(character_id: String, zoom_val: float, duration: float) -> void:
	...
	if rig and is_instance_valid(rig):
		await stage_camera.focus_on(rig.global_position, zoom_val, duration)
...
func focus_camera_between(character_ids: Array[String], duration: float) -> void:
	...
	if not positions.is_empty():
		await stage_camera.focus_between(positions, 100.0, duration)

func reset_camera(duration: float) -> void:
	await stage_camera.reset_to_wide(duration)
