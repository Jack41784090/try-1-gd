...
func add_unit_to_row(row_node: Node3D, unit_index: int, unit_name: String = "Unit") -> Sprite3D:
	...
	print("[Battlefield] Added %s at row '%s', index %d" % [unit_name, row_node.name, unit_index])
...
func update_row_positions(row_node: Node3D) -> void:
	...
	for i in range(children.size()):
		if children[i] is Sprite3D and i < positions.size():
			children[i].position.z = positions[i]
