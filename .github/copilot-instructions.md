...
- **Theme System** (`resources/theme/condor_theme.tres`): Centralized project-level Godot Theme set via `gui/theme/custom` in `project.godot`. Uses EB Garamond variable font (`assets/fonts/eb_garamond.ttf`, OFL licensed) with `FontVariation` resources for regular and bold (embolden=0.4). All visual styling defined via `theme_type_variation` on scene nodes instead of inline `theme_override_*` properties.
  - **Type variations** (Label): HeaderLabel (22px gold bold), SectionLabel (20px gold), SubsectionLabel (16px warm), TurnLabel (20px amber bold), LocationLabel (18px gold bold), StatLabel (16px cream), MetaLabel (14px muted), SmallLabel (12px gray), SpeakerLabel (16px gold), VnSpeakerLabel (22px gold), DialogueLabel (16px cream), AdvancePromptLabel (14px muted), TitleLabel (48px gold bold), CombatTitleLabel (28px gold bold), MoraleDeltaLabel (28px cream), LootHeaderLabel (20px gold), LootItemLabel (16px cream), ConditionLabel (16px cream), DefeatLabel (48px red bold), GameOverDescLabel (18px gray), MoneyLabel (14px gold)
  - **Type variations** (Button): GoldButton (gold text, action_btn styleboxes), NavButton (muted gold, nav_btn styleboxes), CloseButton (red text), ConfirmButton (green text), GoBackButton (muted, go_back styleboxes), ContinueButton (amber, continue styleboxes), FleeButton (red), NegotiateButton (amber), FightButton (green), SmallButton (11px)
  - **Type variations** (PanelContainer): GamePanel (dark semi-transparent, gold border), OverlayPanel (darker, with shadow), SpeechPanel (rounded 12px, shadow), DialoguePanel (rounded 8px), TransparentPanel (fully transparent), ContactPanel (dark, subtle border), ItemPanel (compact, 4px corners), TravelArrowBar (dark gradient-like), CardPanel (dark 0.12 bg, gold border, 4px corners)
  ...
  - **Migration rule**: New .tscn nodes should use `theme_type_variation = &"VariationName"` instead of inline `theme_override_*`. Layout-specific overrides (margins, separation) remain as `theme_override_constants/`. Only texture-based StyleBoxes (e.g., wood_table_diff_4k.jpg panels) stay inline
