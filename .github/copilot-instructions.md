...
- **Contact & Spotting System** (`src/strategy/core/contact/`): HOI4-inspired gradual awareness between squads
  ...
  - `ScoutingFocus` (scouting_focus.gd) — RefCounted, player-configurable filter for contact buildup. `selected_roles: Array[SquadRole]`, `selected_classes: Array[EntityClasses.Types]`. Matching uses OR logic (role match OR class match). `toggle_role()`/`toggle_class()`, presets (`set_preset_aggressive()`, `set_preset_support()`), `clear()`. Stored on `SquadStrategicData.scouting_focus` (persists across turns)
...
- **Inventory & Equipment System** (`src/strategy/core/inventory.gd`, `src/strategy/core/loot_collector.gd`):
  ...
  - `CharacterSocialStats.equipment_weapon: WeaponConfig` / `equipment_armor: ArmorConfig` — per-warrior equipment slots. Used by `_create_entity_config()` to override class defaults in combat
