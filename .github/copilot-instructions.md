...
- **Never programmatically create GUI elements** — all UI elements (Buttons, Labels, Panels, Containers, etc.) must be defined in `.tscn` scene files, not constructed via `X.new()` + `add_child()` in GDScript. Use scene files for layout and `@onready` references for access. The only exception is truly dynamic data-driven lists (e.g., spawning N items for N contacts) — but even then, prefer instancing a reusable `.tscn` template via `preload/load + instantiate()` over manual node construction.
- **Compartmentalize GUI into scenes** — every reusable or logically distinct UI component must be its own `.tscn` file. Overlay views, tab contents, list item templates, cards, and panels should each be a standalone `.tscn` that can be edited independently in the Godot editor. Parent scenes should instance them via `instance=ExtResource(...)` in `.tscn` or `preload().instantiate()` in GDScript for data-driven lists. Never inline large UI trees in a parent scene when they can be split into a subscene.
