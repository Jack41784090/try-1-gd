...
- **Theme System** (`resources/theme/condor_theme.tres`): Centralized project-level Godot Theme set via `gui/theme/custom` in `project.godot`. Uses EB Garamond variable font (`assets/fonts/eb_garamond.ttf`, OFL licensed) with `FontVariation` resources for regular and bold (embolden=0.4). All visual styling defined via `theme_type_variation` on scene nodes instead of inline `theme_override_*` properties.
  ...
  - **Type variations** (Button): GoldButton (gold text, action_btn styleboxes), NavButton (muted gold, nav_btn styleboxes), CloseButton (red text), ConfirmButton (green text), GoBackButton (muted, go_back styleboxes), ContinueButton (amber, continue styleboxes), FleeButton (red), NegotiateButton (amber), FightButton (green)
  - **Type variations** (PanelContainer): GamePanel (dark semi-transparent, gold border), OverlayPanel (darker, with shadow), SpeechPanel (rounded 12px, shadow), DialoguePanel (rounded 8px), TransparentPanel (fully transparent), ContactPanel (dark, subtle border), ItemPanel (compact, 4px corners), TravelArrowBar (dark gradient-like)
